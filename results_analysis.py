...
def plot_scores(scores, title, ylabel, model_name, prompt_labels, filename):
    ...
    bars = ax.bar(
        prompt_labels,
        scores,
        color=['skyblue', 'yellowgreen', 'gold'],
        width=width,
    )
...
model_scores = {
    model: {'general': [], 'alignment': [], 'coverage': []} for model in models
}
...
for model in models:
    ...
    for prompt in prompts:
        ...
        avg_general, avg_alignment, avg_coverage = calculate_average_scores(
            eval_data
        )
...
def plot_model_comparison(scores, title, ylabel, filename, model_labels):
    ...
    bars = ax.bar(
        model_labels,
        scores,
        color=['thistle', 'lightsalmon', 'lightsteelblue'],
        width=width,
    )
...
def plot_optimal_comparison(
    optimal_scores, title, filename, model_labels, prompt_labels
):
    fig, ax = plt.subplots()
...
avg_human_general, avg_human_alignment, avg_human_coverage = (
    calculate_average_scores(human_data)
)


def plot_optimal_vs_human(
    optimal_scores, human_scores, title, filename, model_labels, prompt_labels
):
    fig, ax = plt.subplots()
    ...
    ax.bar(
        index + len(model_labels) * bar_width,
        human_scores_list,
        width=bar_width,
        label=human_label,
        color='gray',
    ...
    for j, score in enumerate(human_scores_list):
        ax.text(
            index[j] + len(model_labels) * bar_width,
            score + 0.02,
            round(score, 2),
            ha='center',
            va='bottom',
